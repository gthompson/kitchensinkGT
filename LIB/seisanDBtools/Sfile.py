...
#                        self.magnitude_agency.append(line[76:79].strip())
            ...
            if line[79] == ' ' and line[1] == 'M':
                ...
                if 'time_residual' in thisarrival:
                    ...
                    p = Pick(time = atime, waveform_id = '..%s..%s' % (asta, achan), onset=onset, phase_hint=aphase, time_errors=tres, backazimuth=180+thisarrival['azimuth'] % 360)
                else:    
                    p = Pick(time = atime, waveform_id = '..%s..%s' % (asta, achan), onset=onset, phase_hint=aphase)
