...
class Catalog(object):
    ...
    def __str__(self):
        ...
        str += 'num evid time                                        lon        lat       depth  mag'
        for i in range(len(self)):
            str += i,'\t',self.evid[i],'\t',self.lon[i],'\t',self.lat[i],'\t: ',self.depth[i],'\t',self.mag[i]
            str += '%4d' % i,' %5d' % self.evid[i],' %s' % self.time[i],' %7.2f' % self.lon[i],' %7.2f' % self.lat[i],' %4.1f' % self.depth[i],' %3.1f' % self.mag[i]
            
    def import_random(self, n):
        """ create N random events """
        dtnow = datetime.datetime.utcnow()
        evid = etype = orid = etime = lon = lat = depth = ml = ms = mb = []
        evid = orid = np.arange(n)
        etype = np.array(['r']*n)
        orid = np.arange(n)
        etime = converttime.utnow() - np.random.random(n) * 86400
        lon = np.random.random(n) * 360 - 180
        lat = np.random.random(n) * 180 - 90
        depth = np.random.random(n) * 30
        ml = np.random.random(n) * 3 - 0.5
        ms = mb = np.ones(n) * -999.9
        self = np.rec.fromarrays([evid, etype, orid, etime, lon, lat, depth, ml, ms, mb],  names = 'evid, etype, orid, etime, lon, lat, depth, ml, ms, mb' )
